...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	private static final String MENSAGEM_ERRO_GENERIA_USUARIO_FINAL = "Ocorreu um erro interno inesperado no sistema. "
	...
	protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers,
		...
		if (body == null) {
			body = Problem.builder().timestamp(LocalDateTime.now()).title(status.getReasonPhrase())
					.status(status.value()).userMessage(MENSAGEM_ERRO_GENERIA_USUARIO_FINAL).build();
		} else if (body instanceof String) {
			body = Problem.builder().timestamp(LocalDateTime.now()).title((String) body).status(status.value())
					.userMessage(MENSAGEM_ERRO_GENERIA_USUARIO_FINAL).build();
		}
	...
	private ResponseEntity<Object> handleInvalidFormatException(InvalidFormatException ex, HttpHeaders headers,
		...
		Problem problem = createProblemBuilder(status, problemType, detail).userMessage(MENSAGEM_ERRO_GENERIA_USUARIO_FINAL).build();
	...
	public ResponseEntity<Object> handleUncaught(Exception ex, WebRequest request) {
		...
		String detail = MENSAGEM_ERRO_GENERIA_USUARIO_FINAL;
		ex.printStackTrace();
	...
	public ResponseEntity<?> handleNegocioException(NegocioException e,
		...
		Problem problem = createProblemBuilder(status, problemType,
				"Ocorreu um erro de negócio, entre em contato com o suporte.").build();
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		List<Problem.Field> problemFields = bindingResult.getFieldErrors().stream().map(fieldError -> {
			...
			return Problem.Field
				.builder().name(fieldError.getField()).userMessage(message).build();
